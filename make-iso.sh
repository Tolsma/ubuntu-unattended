...
# file names & paths
tmp="/home/sander/iso-tmp"  # destination folder to store the final iso file
hostname="ubuntu"
...
read -ep " please enter the github source location: " -i "https://github.com/Tolsma/ubuntu-unattended" github
read -ep " please enter your preferred timezone   : " -i "Europe/Amsterdam" timezone
read -ep " please enter your preferred username   : " -i "sander" username
...
cd $tmp/iso_new
(mkisofs -D -r -V "TTC_UBUNTU" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o $tmp/$new_iso_name . > /dev/null 2>&1) &
spinner $!
